  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(handleSubmit)}>
          ...
          <fieldset className="flex flex-col gap-4" disabled={form.formState.isSubmitting}>
            ...
            <FormField control={form.control} name="description" render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Textarea {...field} rows={5} style={{ height: '120px' }} className="resize-none" />
                </FormControl>
                <FormMessage />
              </FormItem>
            )} />
          ...
          <FormItem>
            <FormControl>
              <MultiImageUploader
                ...
                urlFormatter={(image) => {
                  if (!image.file) {
                    return `https://firebasestorage.googleapis.com/v0/b/fire-homes-5f6dd.firebasestorage.app/o/${encodeURIComponent(image.url)}?alt=media`;
                  }
                  return image.url;
                }}
              />
        ...
        <Button type="submit" className="max-w-md mx-auto mt-5 w-full flex gap-2" disabled={form.formState.isSubmitting}>
          {submitButtonLabel}
        </Button>
      </form>
    </Form>
